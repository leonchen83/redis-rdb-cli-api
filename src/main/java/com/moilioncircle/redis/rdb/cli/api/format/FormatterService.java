...
public interface FormatterService {
    ...
    default Event applySetListPack(Replicator replicator, RedisInputStream in, int version, byte[] key, int type, ContextKeyValuePair context) throws IOException {
        BaseRdbParser parser = new BaseRdbParser(in);
    ...
    default Event applyStreamListPacks3(Replicator replicator, RedisInputStream in, int version, byte[] key, int type, ContextKeyValuePair context) throws IOException {
        BaseRdbParser parser = new BaseRdbParser(in);
        ...
        while (groupCount-- > 0) {
            ...
            while (consumerCount-- > 0) {
                ...
                long seenTime = parser.rdbLoadMillisecondTime();
                // handle <consumerName, seenTime>
                long activeTime = parser.rdbLoadMillisecondTime();
                // handle <activeTime>
                long pel = parser.rdbLoadLen().len;
                while (pel-- > 0) {
                    Stream.ID rawId = new Stream.ID(in.readLong(8, false), in.readLong(8, false));
